...
if [ ! `which python3.8 | grep "/usr/bin/python3.8"` ]; then
	...
	cd ..
	rm -rf Python*
	echo "\nPython3.8.2 installation went successful!\n"
	if [ `which python3.8 | grep "/usr/bin/python3.8"` ]; then
		req
		clean
		echo "\nPython3.8.2 installation with dependencies went successful!\n"
	fi
else
	echo "\nPython3.8.2 is already installed! Skipping step of installation! Checking dependencies...\n"
	req
	clean
	echo "\nPython3.8.2 installation with dependencies went successful!\n"
	clean
